...
class TodoTest extends TestCase {
    ...
    public function testParseContexts() {
        $todo = Todo::fromString("Test @Home");
        $this->thenTodoHas($todo, "Test @Home", false, null, null, null, 0, ['Home'], 0);
        $this->assertEquals("Test", $todo->getCleanText());

        $todo = Todo::fromString("Test @Home and @work");
    ...
    public function testParseMeta() {
        $todo = Todo::fromString("Test color:red");
        $this->thenTodoHas($todo, "Test color:red", false, null, null, null, 0, 0, ['color' => 'red']);
        $this->assertEquals("Test", $todo->getCleanText());

        $todo = Todo::fromString("Test color:red and test:yes");
    ...
    public function testParseComplex() {
        $todo = Todo::fromString("x 2025-01-01 2024-12-01 Just an @example with lots of +details +and more:yes");
        ...
        $todo = Todo::fromString("(Y) 2024-12-01 Another @example @with @lots of +details");
    ...
    public function testParseBrokenFormat() {
        $todo = Todo::fromString("X Not really a done task");
        $this->thenTodoHas($todo, "X Not really a done task",
                           false, null, null, null, 0, 0,0);

        $todo = Todo::fromString("(Y Incorrect priority format");
        $this->thenTodoHas($todo, "(Y Incorrect priority format",
                           false, null, null, null, 0, 0,0);

        $todo = Todo::fromString("2024-02-01 (Y) Wrong order");
        $this->thenTodoHas($todo, "(Y) Wrong order",
                           false, null, '2024-02-01', null, 0, 0,0);

        $todo = Todo::fromString("x Also wrong order 2024-02-01");
    ...
    private function thenStringIs(string $expected, Todo $todo): void {
        $this->assertEquals($expected, "{$todo}", "String does not match");
    }

    private function thenTodoHas(Todo $todo, string $text, bool $done, ?string $priority, ?string $creationDate,
